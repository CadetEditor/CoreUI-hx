...
class DefaultPropertyInspectorDataDescriptor implements IPropertyInspectorDataDescriptor
{
	...
	public function getFields(object : Dynamic) : Array<Dynamic>
	{
		var description : FastXML = describeType(object);
		var fields : Array<Dynamic> = [];
		for (node/* AS3HX WARNING could not determine type for var: node exp: ECall(EField(EIdent(description),children),[]) type: null */ in description.nodes.children()) {
			if (node.name() != "accessor" && node.name() != "variable") continue;
			var metadata : FastXML = FastXML.filterNodes(node.metadata, function(x:FastXML) {
                if(x.att.name == "Inspectable") return true;
                return false;
            }).get(0);
			if (metadata == null) continue;
			var field : PropertyInspectorField = new PropertyInspectorField();
			field.category = getClassName(object);
			field.property = Std.string(node.att.name);
			field.hosts = [object];  
			// If the property is an inspectable list, then populate the fields with the children of that list too.  
			if (Std.is(object[field.property], Proxy)) {
				var children : Dynamic = cast((object[field.property]), Proxy);
				for (i in 0...children.length) {
					var child : Dynamic = children[i];
					var childFields : Array<Dynamic> = getFields(child);
					childFields = childFields.sortOn("property");
					fields = fields.concat(childFields);
                }
            } else {  
				// Parse each metadata argument into the field  
				for (i in 0...metadata.nodes.arg.length()) {
					var argNode : FastXML = metadata.nodes.arg.get(i);
					var key : String = Std.string(argNode.att.key);
					var value : String = Std.string(argNode.att.value);  
					...
					if (key == "editor") {
						...
						field.priority = as3hx.Compat.parseInt(value);
                    }
                    ...
                    // values then get passed to the editor when created.
                        ...
                        } else if (Math.isNaN(Std.parseFloat(value)) == false) {
							field.editorParameters[key] = Std.parseFloat(value);
				...
				if (field.editorID == null || field.editorID == "") {
					var value2 : Dynamic = object[field.property];
					if (Std.is(value2, Float)) {
						field.editorID = "NumberInput";
                    } else if (Std.is(value2, Bool)) {
	...
	private static function getClassName(object : Dynamic) : String
	{
		var classPath : String = flash.utils.getQualifiedClassName(object).replace("::", ".");
		if (classPath.indexOf(".") == -1) return; 
		classPath; var split : Array<Dynamic> = classPath.split("."); 
